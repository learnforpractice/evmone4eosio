  ...
  namespace rlp
  {
    ...
    namespace encode_details
    {
      ...
      inline ByteString to_byte_string(uint64_t n)
      {
        ...
        for (size_t byte_index = 0; byte_index < sizeof(n); ++byte_index)
        {
          ...
          const uint8_t current_byte = uint8_t((mask & n) >> offset);
      ...
      inline void prefix_multiple_length(size_t total_length, ByteString& bs)
      {
        ...
        if (total_length <= 55)
        {
          bs.insert(bs.begin(), uint8_t(0xc0 + total_length));
          return;
        }
        ...
        const uint8_t length_of_total_length = uint8_t(total_length_as_bytes.size());
      ...
      inline void length_to_byte_string(size_t total_length, ByteString& bs)
      {
        // "If the total payload of a list (i.e. the combined length of all its
        // items being RLP encoded) is 0-55 bytes long, the RLP encoding
        // consists of a single byte with value 0xc0 plus the length of the list
      ...
      ByteString encode_single(const T& t)
      {
        ...
        if (length <= 55)
        {
          bytes.insert(bytes.begin(), uint8_t(0x80 + length));
          return bytes;
        }
        ...
        const uint8_t length_of_length = uint8_t(length_as_bytes.size());
    ...
    namespace decode_details
    {
        ...
        const uint8_t*& data, size_t& size)
      {
        if (size == 0)
        {
          EOSIO_THROW("Trying to decode length: got empty data");
        }
      ...
      std::tuple<Ts...> decode_impl(const uint8_t*& data, size_t& size)
      {
          ...
          sizeof...(Ts) == 1 && !is_tuple<std::decay_t<Ts>...>::value)
        {
          ...
          {
            EOSIO_THROW("Expected single item, but data encodes a list");
          }
        ...
        {
          EOSIO_THROW("Expected list item, but data encodes a single item");
        }
